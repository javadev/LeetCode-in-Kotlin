...
class Solution {
    ...
    companion object {
        fun calcPerms(root: TreeNode?, fact: LongArray): TreeInfo {
            val left: TreeInfo
            val right: TreeInfo
        ...
        fun getModDivision(a: Long, b1: Long, b2: Long, m: Long): Long {
            val b = b1 * b2
            val inv = getInverse(b, m)
            return inv * a % m
        }

        fun getInverse(b: Long, m: Long): Long {
            val inv = getInverseExtended(b, m)
            return (inv.x % m + m) % m
        }

        fun getInverseExtended(a: Long, b: Long): Inverse {
            if (a == 0L) {
                return Inverse(0, 1)
            }
        ...
        fun addInTree(x: Int, root: TreeNode?) {
            if (root!!.`val` > x) {
                if (root.left != null) {
